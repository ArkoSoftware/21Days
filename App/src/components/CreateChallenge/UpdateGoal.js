...
import { CompleteSubmission } from "./functions/function2";
import Loading from "../Loading";
const windowHeight = Dimensions.get("window").height;

const frequency = ["Monthly", "Daily", "Weekly", "Hourly"];

const UpdateGoal = ({ open, setOpen, group, groupId, item }) => {
  const [isLoading, setIsLoading] = useState(false);
  // React Hooks
  const [image, setImage] = useState(item[1].image);
  const [habit, setHabit] = useState(false);
  const [taskTitle, setTaskTitle] = useState(item[1].taskTitle);
  const [category, setCategory] = useState(item[1].category);
